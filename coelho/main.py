...
async def search(q: str):
    ...
    if doc.exists:
        title, author, pub_date, binding, img = itemgetter(
            "title", "author", "pub_date", "binding", "img"
        )(doc.to_dict())
    ...
    try:
        ...
        # fetch description and subject information from loc.goc

        loc_items = [
            result["item"]
            for result in itemgetter("results")(
                requests.get(
                    f"https://www.loc.gov/search/?all=True&fo=json&q=${q}"
                ).json()
            )
        ]
        ...
        if len(loc_items) > 0:
            ...
            summary = ". ".join(loc_item["summary"])

            def format_subject(raw: str) -> str:
                try:
                    sub_subject, subject = [part.strip() for part in raw.split("--")]
                    return f"{subject} / {sub_subject}"
                except:
                    try:
                        return " / ".join([part.strip().capitalize() for part in raw.split("/")])
                    except:
                        return raw

            subjects = [format_subject(subject) for subject in loc_item["subjects"]]
